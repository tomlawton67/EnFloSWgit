...
// Developed and tested by Paul Nathan, 27/06/2015. Latest modification 06/10/2016.
...
const unsigned long PollPeriod = 3750; //us
...
const long Baud = 1000000; // 1000000 // 476000 #!#!#!#!# CHANGE THIS NUMBER TO DESIRED BAUD RATE #!#!#!#!#

const char Ident[4] = {'A', 'D', 'C', 't'};

#define spiAD7173 SPISettings(20E6, MSBFIRST, SPI_MODE3)
...
void loop()
{
  while (true)
  {
    ...
    if ((Now - LastPoll) > PollPeriod)
    {
      ...
      SPI.beginTransaction(spiAD7173);
      PORTF &= ~Pin_CS; // Low
...
void ConfigureAD7173()
{
  ...
  SPI.transfer(0b00001110); // 0 SINC3_MAP0 off, 000, 1 ENHFILTEN0 on, 110 ENHFILT0 92db 60ms
  SPI.transfer(0b00010000); // 0, 00 Sinc5 + sinc1, 10000 49.68 Hz data rate <<<<<<<<<<<<<<<<<<<<<<<<<<<<< DATA RATE SET HERE!
  PORTF |= Pin_CS; // High
...
void SerialLoop()
{
  ...
  const byte N = Serial1.available();
  if (N > 63) PORTD |= Pin_buf;
  if (N >= L) // only do something if data is waiting from RS-485 network
  {
